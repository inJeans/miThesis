...
Originally printed in il nuova cimento in 19?? it was subsequently translated and reprinted in blah in 19??.
...
It is not always possible to obtain an integral solution, but when possible it provides a global solution from which all asymptotic limits can be obtained, and hence connect solutions from one end of the domain to another. 
...
If we begin with \autoref{eq:Majprob} and make the assumption that atoms who's probability of flipping is great than 50\% (that is $\left\vert g \right\vert^{2} > 0.5$) leave the trap we can find the threshold for flipping with
\begin{equation}
    2 \log_{e}2 = \pi k.
\end{equation}
